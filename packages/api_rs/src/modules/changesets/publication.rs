...
pub(crate) fn publish_sync(
    ...
    check_node_version_conflicts(c, id, world_id)?;
    check_way_version_conflicts(c, id, world_id)?;
    check_relation_version_conflicts(c, id, world_id)?;
    validate_publication_topology(c, id, world_id)?;
    ...
    for (node_id, x, y, z, tags, by) in node_updates {
        update(
            core::nodes::table
                .filter(core::nodes::id.eq(node_id))
                .filter(core::nodes::world_id.eq(world_id)),
        )
        .set((
    ...
    for (relation_id, relation_type, tags, by) in relation_updates {
        update(
            core::relations::table
                .filter(core::relations::id.eq(relation_id))
                .filter(core::relations::world_id.eq(world_id)),
        )
        .set((
            core::relations::relation_type
                .eq(relation_type.ok_or_else(|| std::io::Error::other("invalid relation draft"))?),
    ...
    delete(
        derived::way_geometries::table.filter(
            derived::way_geometries::way_id.eq_any(
                core::ways::table
                    .filter(core::ways::world_id.eq(world_id))
                    .select(core::ways::id),
            ),
    ...
    .execute(c)?;
    sql_query("WITH linework AS (SELECT w.id,w.geometry_kind,w.version,ST_MakeLine(ARRAY(SELECT ST_SetSRID(ST_MakePoint(n.mc_x,n.mc_z),0) FROM core.way_nodes wn JOIN core.nodes n ON n.id=wn.node_id AND n.world_id=w.world_id WHERE wn.way_id=w.id AND wn.world_id=w.world_id AND n.deleted_at IS NULL ORDER BY wn.seq)) geom,(SELECT count(DISTINCT wn.node_id) FROM core.way_nodes wn WHERE wn.way_id=w.id AND wn.world_id=w.world_id) distinct_nodes FROM core.ways w WHERE w.world_id=$1 AND w.deleted_at IS NULL) INSERT INTO derived.way_geometries(way_id,geom,bbox,source_version) SELECT id,CASE WHEN geometry_kind='area' THEN ST_MakePolygon(geom) ELSE geom END,ST_Envelope(CASE WHEN geometry_kind='area' THEN ST_MakePolygon(geom) ELSE geom END),version FROM linework WHERE geometry_kind='line' OR (distinct_nodes>=3 AND ST_IsClosed(geom) AND ST_IsValid(ST_MakePolygon(geom)))").bind::<diesel::sql_types::BigInt, _>(world_id).execute(c)?;
    delete(
        derived::relation_geometries::table.filter(
            derived::relation_geometries::relation_id.eq_any(
                core::relations::table
                    .filter(core::relations::world_id.eq(world_id))
                    .select(core::relations::id),
            ),
    ...
    .execute(c)?;
    sql_query("WITH grouped AS (SELECT r.id,r.version,ST_Union(g.geom) FILTER (WHERE m.role IS NULL OR m.role='outer') outer_geom,ST_Union(g.geom) FILTER (WHERE m.role='inner') inner_geom FROM core.relations r JOIN core.relation_members m ON m.relation_id=r.id AND m.world_id=r.world_id AND m.member_type='way' JOIN core.ways w ON w.id=m.member_id AND w.world_id=r.world_id AND w.geometry_kind='area' AND w.deleted_at IS NULL JOIN derived.way_geometries g ON g.way_id=m.member_id WHERE r.world_id=$1 AND r.deleted_at IS NULL AND r.relation_type='multipolygon' AND NOT EXISTS (SELECT 1 FROM core.relation_members bad WHERE bad.relation_id=r.id AND bad.world_id=r.world_id AND (bad.member_type<>'way' OR bad.role IS NOT NULL AND bad.role NOT IN ('outer','inner'))) GROUP BY r.id,r.version) INSERT INTO derived.relation_geometries(relation_id,geom,bbox,source_version) SELECT id,ST_Multi(ST_CollectionExtract(CASE WHEN inner_geom IS NULL THEN outer_geom ELSE ST_Difference(outer_geom,inner_geom) END,3)),ST_Envelope(CASE WHEN inner_geom IS NULL THEN outer_geom ELSE ST_Difference(outer_geom,inner_geom) END),version FROM grouped WHERE outer_geom IS NOT NULL AND NOT ST_IsEmpty(outer_geom) AND ST_IsValid(CASE WHEN inner_geom IS NULL THEN outer_geom ELSE ST_Difference(outer_geom,inner_geom) END)").bind::<diesel::sql_types::BigInt, _>(world_id).execute(c)?;
    ...
    let row = update(
        core::changesets::table
            .filter(core::changesets::id.eq(id))
